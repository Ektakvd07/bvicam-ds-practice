...
import static Watertank.Watertank2.watercontain;
    ...
    sum=0;
        ...
        {lmax=arr[0];
         rmax=arr[n-1];
            ...
            if(lmax<rmax)
            {sum+=lmax-arr[i];//assign arr2[i] on condition lmax<rmax
                
            }
            else if(rmax<=lmax)
                sum+=rmax-arr[i];//assign arr2[i] on condition lmax>=rmax
